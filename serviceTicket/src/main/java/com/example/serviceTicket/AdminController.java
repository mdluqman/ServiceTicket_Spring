...
public class AdminController {
	...
	/*
	 * this gets the list of deparments with there respective deptartment-id's
	 * so that they can be displayed on the registraion page....
	 * and it directs to registration page(Helping admin to perform registration functionality
	 * */
	@RequestMapping(value = "/registerdisplay")
	public ModelAndView getdept() {
		String port = environment.getProperty("local.server.port");
		final String uri = "http://localhost:"+port+"/Admin/register";
	...
	/*
	 * Takes you to the RestController then to busness class and then to DAO class that has
	 * function which is used to register the respective CLIENT(USER)
	 * ie function that saves data in the table is called through this
	 * */
	...
	/*
	 * this is used to get the list of type of users from the usertypeinfo table in the database
	 * this list is needed so that admin can select the type of client/user he wants to register. 
	 * */
	...
	/*
	 * this is used to direct to the functions that'll display the list of clients according to the type selected
	 * */
	...
	public ModelAndView viewclient(usertypeinfo u) {
		...
		if (u.getUserTypeId() == 1 || u.getUserTypeId() == 3) {
			...
			if (u.getUserTypeId() == 2) {
				System.out.println("hi to get admin details with type:" +u.getUserTypeId());
	...
	/*
	 * this is used to call the delete function by which admin can either delte a serviceEngineer
	 * or a EndUser 
	 * or an Admin
	 * */
	...
	public ModelAndView delete(UserBean user, @RequestParam("name") String username, @RequestParam("id2") int y,
		...
		System.out.println("delete in controller" + username + " " + y + " " + seid);
		...
		RestTemplate restTemplate = new RestTemplate();
		String message = restTemplate.postForObject(uri, user, String.class);
		System.out.println(message + "delete performed for authentication");
		ModelAndView mv = new ModelAndView("/Admin");
		mv.addObject("message", message);
		return mv;
		}
